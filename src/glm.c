...
void glmSimpleVertexNormals(GLMmodel* model)
{
	GLMnode*    node;
	GLMnode*    tail;
	GLMnode** members;
...
//	printf("average normals per vertex\n");
...
//			printf("%d %f\n", T(node->index).findex, model->facetnorms[3 * T(node->index).findex + 0]);
//			printf("%d %f %f %f\n", T(node->index).findex, average[0], average[1], average[2]);
			node = node->next;
		}
